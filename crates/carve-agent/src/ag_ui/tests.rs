...
fn test_request_with_large_messages() {
    ...
    let request = RunAgentRequest::new("t", "r").with_message(AGUIMessage::user(&large_content));
...
async fn test_interaction_response_plugin_blocks_denied() {
    ...
    let plugin = InteractionResponsePlugin::new(vec![], vec!["permission_write_file".to_string()]);
...
async fn test_interaction_response_plugin_allows_approved() {
    ...
    let plugin = InteractionResponsePlugin::new(vec!["permission_read_file".to_string()], vec![]);
...
async fn test_interaction_response_plugin_ignores_without_pending_interaction() {
    ...
    let plugin = InteractionResponsePlugin::new(vec!["permission_delete_file".to_string()], vec![]);
...
fn test_aguitooldef_with_parameters() {
    ...
    let tool = AGUIToolDef::backend("write_file", "Write to file").with_parameters(schema.clone());
...
fn test_interaction_response_all_approved_string_variants() {
    let approved_strings = vec![
        "true", "yes", "approved", "allow", "confirm", "ok", "accept", "TRUE", "YES", "APPROVED",
        "ALLOW", "CONFIRM", "OK", "ACCEPT", "True", "Yes", "Approved", "Allow", "Confirm", "Ok",
        "Accept",
    ];
...
fn test_interaction_response_all_denied_string_variants() {
    let denied_strings = vec![
        ...
        "DENY", "REJECT", "CANCEL", "ABORT", "False", "No", "Denied", "Deny", "Reject", "Cancel",
        "Abort",
    ];
...
async fn test_run_agent_stream_run_id_consistent_in_finished() {
    ...
    let client = Client::default();
    let config = AgentConfig::default().with_plugin(Arc::new(SkipPlugin) as Arc<dyn AgentPlugin>);
...
async fn test_run_agent_stream_with_request_model_override() {
    ...
    let config =
        AgentConfig::new("original-model").with_plugin(Arc::new(plugin) as Arc<dyn AgentPlugin>);
...
fn test_session_has_message_id_ignores_none_ids() {
    let thread = Thread::new("s1").with_message(crate::types::Message::user("no id on this one"));
...
fn test_apply_agui_request_imports_user_messages_without_id() {
    ...
    let request = RunAgentRequest::new("t1", "r1").with_message(AGUIMessage::user("no-id message"));
...
fn test_idless_user_messages_still_not_deduplicated() {
    ...
    let thread = Thread::new("s1").with_message(crate::types::Message::user("existing"));
    let request = RunAgentRequest::new("t1", "r1").with_message(AGUIMessage::user("no-id input"));
...
fn test_agui_run_finish_without_text_just_finished() {
    ...
    let ev = crate::stream::AgentEvent::RunFinish {
        thread_id: "th".into(),
        run_id: "run".into(),
...
fn test_agui_tool_call_ready_produces_tool_call_end() {
    let mut ctx = AGUIContext::new("th".into(), "run".into());
    let ev = crate::stream::AgentEvent::ToolCallReady {
        id: "tc-1".into(),
        name: "search".into(),
        arguments: json!({"query": "rust"}),
    };
...
fn test_interaction_to_agui_events_with_parameters() {
    let interaction =
        Interaction::new("i2", "tool:doStuff").with_parameters(json!({"key": "value", "count": 5}));
    let events = interaction.to_ag_ui_events();
...
fn test_prepare_request_runtime_appends_after_existing_plugins() {
    ...
    let (config, _thread, _tools) = super::prepare_request_runtime(config, thread, tools, &request);
    let plugin_ids: Vec<&str> = config.plugins.iter().map(|p| p.id()).collect();
...
fn test_request_last_user_message_none() {
    let request = RunAgentRequest::new("t1", "r1").with_message(AGUIMessage::system("only system"));
    assert_eq!(request.last_user_message(), None);
}
...
fn test_state_delta_move_operation() {
    let delta = AGUIEvent::state_delta(vec![json!({"op": "move", "from": "/old", "path": "/new"})]);
    let json = serde_json::to_string(&delta).unwrap();
    let back: AGUIEvent = serde_json::from_str(&json).unwrap();
    if let AGUIEvent::StateDelta { delta, .. } = back {
...
fn test_text_message_start_tolerates_extra_fields() {
    let json =
        r#"{"type":"TEXT_MESSAGE_START","messageId":"m1","role":"assistant","extraField":true}"#;
    let event: AGUIEvent = serde_json::from_str(json).unwrap();
