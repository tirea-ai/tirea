...
pub(super) struct AgentRunRecord {
    ...
    pub(super) thread: crate::contracts::conversation::AgentState,
...
impl AgentRunManager {
    ...
    pub(super) async fn owned_record(
        ...
        owner_thread_id: &str,
        run_id: &str,
    ) -> Option<crate::contracts::conversation::AgentState> {
        let runs = self.runs.lock().await;
        let rec = runs.get(run_id)?;
        if rec.owner_thread_id != owner_thread_id {
            return None;
...
fn last_assistant_message(thread: &crate::contracts::conversation::AgentState) -> Option<String> {
    thread
        .messages
        .iter()
        .rev()
