use super::*;
pub(super) fn parent_run_id_from_thread(thread: Option<&crate::thread::Thread>) -> Option<String> {
    thread
...
pub(super) fn set_persisted_run(ctx: &Context<'_>, run_id: &str, run: AgentRunState) {
    let agent = ctx.state::<AgentState>(AGENT_STATE_PATH);
    agent.agent_runs_insert(run_id.to_string(), run);
}

pub(super) fn parse_persisted_runs(ctx: &Context<'_>) -> HashMap<String, AgentRunState> {
    let agent = ctx.state::<AgentState>(AGENT_STATE_PATH);
