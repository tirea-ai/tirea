...
async fn agent_run_tool_requires_scope_context() {
    ...
    let doc = json!({});
    let ctx = crate::contracts::AgentState::new_runtime(&doc, "call-1", "tool:agent_run");
    let result = tool
        .execute(
...
async fn background_stop_then_resume_completes() {
    ...
    let doc = json!({});
    let rt = caller_scope();
    let ctx = crate::contracts::AgentState::new_runtime(&doc, "call-run", "tool:agent_run")
        .with_runtime_scope(Some(&rt));
    ...
    let stop_ctx = crate::contracts::AgentState::new_runtime(&doc, "call-stop", "tool:agent_stop")
        .with_runtime_scope(Some(&rt));
...
async fn manager_stop_tree_stops_descendants() {
    ...
    manager
        .put_running(
            ...
            "agent-a".to_string(),
            None,
            AgentState::new("parent-run-thread"),
            None,
        )
        .await;
    manager
        .put_running(
            ...
            AgentState::new("child-run-thread"),
            None,
        )
        .await;
    manager
        .put_running(
            ...
            AgentState::new("grandchild-run-thread"),
            None,
        )
        .await;
    manager
        .put_running(
            ...
            "agent-b".to_string(),
            Some("parent-run".to_string()),
            AgentState::new("other-owner-thread"),
            None,
        )
        .await;
...
async fn agent_run_tool_resumes_from_persisted_state_without_live_record() {
    ...
    let child_thread = crate::contracts::conversation::AgentState::new("child-run")
...
async fn agent_stop_tool_stops_descendant_runs() {
    ...
    let os_thread = AgentState::new("owner-thread");

    let parent_thread = AgentState::new("parent-s");
    let child_thread = AgentState::new("child-s");
    let grandchild_thread = AgentState::new("grandchild-s");
    ...
    rt.set(TOOL_SCOPE_CALLER_THREAD_ID_KEY, os_thread.id.clone())
        .unwrap();
    let ctx = crate::contracts::AgentState::new_runtime(&doc, "call-stop", "tool:agent_stop")
        .with_runtime_scope(Some(&rt));
