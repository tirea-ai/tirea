...
use carve_agent::contracts::state_types::InteractionResponse;
...
async fn test_session_with_tool_workflow() {
    ...
    let thread = thread.with_message(carve_agent::engine::convert::assistant_tool_calls(
        &result1.text,
...
async fn test_session_storage_roundtrip() {
    ...
    // Load session
    let loaded = storage.load_agent_state("storage-test").await.unwrap().unwrap();
...
async fn test_file_storage_session_persistence() {
    ...
    // Load and verify
    let loaded = storage.load_agent_state("persist-test").await.unwrap().unwrap();
    let state = loaded.rebuild_state().unwrap();
...
async fn test_session_snapshot_and_continue() {
    ...
    // Save and load
    storage.save(&thread).await.unwrap();
    let loaded = storage.load_agent_state("snapshot-test").await.unwrap().unwrap();
...
async fn test_storage_list_and_delete() {
    ...
    storage.save(&ConversationAgentState::new("thread-1")).await.unwrap();
    storage.save(&ConversationAgentState::new("thread-2")).await.unwrap();
    storage.save(&ConversationAgentState::new("thread-3")).await.unwrap();
...
async fn test_large_session_storage_roundtrip() {
    ...
    // Load
    let loaded = storage
        .load_agent_state("large-storage-test")
        .await
        .unwrap()
        .unwrap();
...
async fn test_session_recovery_after_partial_save() {
    ...
    let recovered = storage.load_agent_state("recovery-test").await.unwrap().unwrap();
...
async fn test_session_incremental_checkpoints() {
    ...
    // Verify final state
    let loaded = storage
        .load_agent_state("checkpoint-test")
        .await
        .unwrap()
        .unwrap();
    let state = loaded.rebuild_state().unwrap();
...
async fn test_incremental_checkpoints_via_append() {
    use carve_agent::contracts::storage::{
        CheckpointReason, AgentChangeSet, AgentStateSync, AgentStateWriter,
    };
    ...
    let head = AgentStateReader::load(&storage, "append-test")
        .await
        .unwrap()
        .unwrap();
...
async fn test_session_recovery_with_snapshot() {
    ...
    let mut thread = storage
        .load_agent_state("snapshot-recovery")
        .await
        .unwrap()
        .unwrap();
    for _ in 0..25 {
...
use carve_agent::runtime::streaming::StreamCollector;
...
fn test_stream_result_from_partial_response() {
    ...
    let msg = carve_agent::engine::convert::assistant_message(&result.text);
...
async fn test_session_resilient_to_tool_errors() {
    ...
    for (i, result) in tool_results.iter().enumerate() {
        thread = thread.with_message(carve_agent::engine::convert::tool_response(
            format!("call_{}", i),
            result,
        ));
    }
...
async fn test_storage_error_recovery() {
    ...
    let result = storage.load_agent_state("non-existent").await;
    assert!(result.is_ok());
    assert!(result.unwrap().is_none());

    // Original session should still be loadable
    let loaded = storage.load_agent_state("valid-thread").await.unwrap().unwrap();
...
async fn test_concurrent_errors_dont_corrupt_storage() {
    ...
    let thread = ConversationAgentState::new("concurrent-test").with_message(Message::user("Initial"));
    storage.save(&thread).await.unwrap();
    ...
    for i in 0..50 {
        let storage = Arc::clone(&storage);
        let handle: tokio::task::JoinHandle<
            Result<Option<AgentState>, carve_agent::contracts::storage::AgentStateStoreError>,
        > = tokio::spawn(async move {
            if i % 3 == 0 {
                // Load
                storage.load_agent_state("concurrent-test").await
            } else {
    ...
    let final_thread = storage
        .load_agent_state("concurrent-test")
        .await
        .unwrap()
        .unwrap();
...
fn test_stream_output_tool_call_delta_coverage() {
    use carve_agent::runtime::streaming::StreamOutput;
...
async fn test_file_storage_corrupted_json() {
    ...
    let result = storage.load_agent_state("corrupted").await;
    assert!(result.is_err());

    match result {
        Err(carve_agent::contracts::storage::AgentStateStoreError::Serialization(msg)) => {
...
fn test_tool_error_variants_display() {
    use carve_agent::contracts::traits::tool::ToolError;
...
fn test_storage_error_variants() {
    use carve_agent::contracts::storage::AgentStateStoreError;
    ...
    let serialization_error = AgentStateStoreError::Serialization("Invalid JSON at line 5".to_string());
    ...
    let not_found = AgentStateStoreError::NotFound("thread-123".to_string());
...
async fn test_sequential_execution_with_conflicting_patches() {
    use carve_agent::engine::tool_execution::execute_tools_sequential;
...
async fn test_parallel_execution_state_isolation() {
    use carve_agent::engine::tool_execution::execute_tools_parallel;
...
async fn test_sequential_vs_parallel_execution_difference() {
    use carve_agent::engine::tool_execution::{execute_tools_parallel, execute_tools_sequential};
...
async fn test_e2e_multi_step_with_state() {
    ...
    thread = thread.with_message(carve_agent::engine::convert::assistant_tool_calls(
        &response1.text,
...
async fn test_e2e_tool_failure_handling() {
    let thread = ConversationAgentState::new("e2e-failure").with_message(Message::user("Call a non-existent tool"));
...
async fn test_e2e_session_persistence_restore() {
    ...
    let mut loaded = storage.load_agent_state("e2e-persist").await.unwrap().unwrap();
    ...
    loaded = loaded.with_message(carve_agent::engine::convert::assistant_tool_calls(
        &response2.text,
        response2.tool_calls.clone(),
    ));
...
async fn test_e2e_long_conversation() {
    ...
    let storage = MemoryStore::new();
    storage.save(&thread).await.unwrap();
    let loaded = storage.load_agent_state("e2e-long").await.unwrap().unwrap();
...
async fn test_e2e_pending_tool_in_session_flow() {
    let thread = ConversationAgentState::new("pending-test").with_message(Message::user("Delete all files"));
...
async fn test_concurrent_session_modifications() {
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            // Load and verify
            let loaded = storage.load_agent_state(&thread_id).await.unwrap().unwrap();
...
async fn test_concurrent_read_write_same_session() {
    ...
    let thread = ConversationAgentState::new("shared-thread").with_message(Message::user("Initial message"));
    storage.save(&thread).await.unwrap();
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            if i % 2 == 0 {
                // Reader
                let loaded = storage.load_agent_state("shared-thread").await.unwrap();
                loaded.is_some()
            } else {
                // Writer (updates the thread)
                let mut thread = storage.load_agent_state("shared-thread").await.unwrap().unwrap();
    ...
    let final_thread = storage.load_agent_state("shared-thread").await.unwrap().unwrap();
...
async fn test_storage_session_not_found() {
    ...
    let result = storage.load_agent_state("nonexistent").await.unwrap();
...
async fn test_storage_overwrite_session() {
    ...
    // Create and save
    let session1 = ConversationAgentState::new("overwrite-test").with_message(Message::user("First version"));
    ...
    let loaded = storage
        .load_agent_state("overwrite-test")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(loaded.message_count(), 2);
...
async fn test_file_storage_special_characters_in_id() {
    ...
    let thread = ConversationAgentState::new("session_with-special.chars_123").with_message(Message::user("Test"));

    storage.save(&thread).await.unwrap();
    let loaded = storage
        .load_agent_state("session_with-special.chars_123")
        .await
        .unwrap();
...
async fn test_file_storage_concurrent_writes_different_sessions() {
    ...
    for i in 0..20 {
        let loaded = storage
            .load_agent_state(&format!("session_{}", i))
            .await
            .unwrap();
...
async fn test_file_storage_concurrent_writes_same_session() {
    ...
    for i in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            let thread =
                ConversationAgentState::new("shared_session").with_message(Message::user(&format!("Write {}", i)));
            s.save(&thread).await.unwrap();
        }));
    }
    ...
    let loaded = storage.load_agent_state("shared_session").await.unwrap();
...
async fn test_file_storage_read_write_interleaved() {
    ...
    let final_thread = storage.load_agent_state("interleaved").await.unwrap().unwrap();
    assert!(final_thread.message_count() >= 1);
}
...
async fn test_storage_empty_session() {
    ...
    let loaded = storage.load_agent_state("empty-thread").await.unwrap().unwrap();
...
async fn test_storage_large_state() {
    ...
    let loaded = storage.load_agent_state("large-state").await.unwrap().unwrap();
    let state = loaded.rebuild_state().unwrap();
...
fn test_session_with_all_message_types() {
    ...
    assert_eq!(thread.messages[4].role, Role::Tool);
...
async fn test_e2e_system_prompt_in_session() {
    ...
    let loaded = storage
        .load_agent_state("system-prompt-test")
        .await
        .unwrap()
        .unwrap();
...
fn frontend_plugin_from_request(request: &RunAgentRequest) -> TestFrontendToolPlugin {
...
async fn test_e2e_permission_suspend_with_real_tool() {
    use carve_agent::runtime::loop_runner::{execute_tools_with_plugins, tool_map, AgentLoopError};
    ...
    let tools = tool_map([IncrementTool]);
    let plugins: Vec<Arc<dyn carve_agent::contracts::agent_plugin::AgentPlugin>> =
...
async fn test_e2e_permission_deny_blocks_via_execute_tools() {
    ...
    let resume_plugins: Vec<Arc<dyn carve_agent::contracts::agent_plugin::AgentPlugin>> =
...
fn test_base_event_fields_with_raw_event() {
    use carve_protocol_ag_ui::BaseEventFields;
...
fn test_event_with_raw_event_passthrough() {
    ...
    let event = AGUIEvent::TextMessageContent {
        ...
        delta: "Hello".into(),
        base: carve_protocol_ag_ui::BaseEventFields {
            timestamp: Some(1234567890),
...
fn test_request_error_invalid_field() {
    use carve_protocol_ag_ui::RequestError;
