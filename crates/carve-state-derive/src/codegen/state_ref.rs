    ...
    let method = match &kind {
        FieldKind::Nested => {
            if field.flatten {
                quote! {
                    ...
                    pub fn #field_name(&self) -> <#field_ty as ::carve_state::State>::Ref<'a> {
                        <#field_ty as ::carve_state::State>::state_ref(
                            self.doc,
                            self.base.clone(),
                            ::carve_state::PatchSink::new(self.sink.inner()),
                        )
                    }
                }
            } else {
        ...
        FieldKind::Option(inner) if inner.is_nested() => {
            ...
            quote! {
                ...
                pub fn #field_name(&self) -> <#inner_ty as ::carve_state::State>::Ref<'a> {
                    let path = self.base.clone().key(#json_key);
                    <#inner_ty as ::carve_state::State>::state_ref(
                        self.doc,
                        path,
                        ::carve_state::PatchSink::new(self.sink.inner()),
                    )
        ...
        FieldKind::Option(inner) if inner.is_nested() => {
            ...
            quote! {
                ...
                pub fn #set_name(&self, value: #field_ty) {
                    let path = self.base.clone().key(#json_key);
                    self.sink.collect(::carve_state::Op::Set {
                        path,
            ...
            if inner.is_nested() {
                quote! {
                    ...
                    pub fn #push_name(&self, value: #inner_ty) {
                        let path = self.base.clone().key(#json_key);
                        self.sink.collect(::carve_state::Op::Append {
                            path,
                            value: ::serde_json::to_value(&value).unwrap_or(::serde_json::Value::Null),
                        });
                    }
    ...
    for field in fields {
        ...
        methods.extend(quote! {
            ...
            pub fn #delete_name(&self) {
                let path = self.base.clone().key(#json_key);
                self.sink.collect(::carve_state::Op::Delete { path });
            }
        });
    }
        ...
        if is_int {
            methods.extend(quote! {
                ...
                pub fn #increment_name(&self, amount: i64) {
                    let path = self.base.clone().key(#json_key);
                    self.sink.collect(::carve_state::Op::Increment {
                        path,
                        amount: ::carve_state::Number::Int(amount),
                    });
                }
                ...
                pub fn #decrement_name(&self, amount: i64) {
                    let path = self.base.clone().key(#json_key);
                    self.sink.collect(::carve_state::Op::Decrement {
                        path,
                        amount: ::carve_state::Number::Int(amount),
                    });
                }
            ...
            methods.extend(quote! {
                ...
                pub fn #increment_name(&self, amount: f64) {
                    ...
                    self.sink.collect(::carve_state::Op::Increment {
                        path,
                        amount: ::carve_state::Number::Float(amount),
                    });
                }
