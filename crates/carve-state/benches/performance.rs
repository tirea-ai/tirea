//! Performance benchmarks for carve-state operations.
//!
//! Run with: cargo bench --package carve-state

use carve_state::{apply_patch, path, Op, Patch};
...
fn bench_apply_patch_nested(c: &mut Criterion) {
    ...
    for depth in [5, 10, 20, 50] {
        ...
        let patch = Patch::new().with_op(Op::set(
            carve_state::Path::from_segments(
                path_segments
                    .iter()
                    .map(|s| carve_state::Seg::Key(s.clone()))
                    .collect(),
            ),
            json!(999),
        ));
