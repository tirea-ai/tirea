//! Edge case tests for carve-state.

use carve_state::{apply_patch, path, CarveError, JsonWriter, Op, Patch, Path};
use serde_json::json;
...
fn test_increment_non_number_error() {
    ...
    assert!(matches!(
        result,
        Err(CarveError::NumericOperationOnNonNumber { .. })
    ));
}
...
fn test_remove_from_non_array_error() {
    ...
    let result = apply_patch(&doc, &patch);
    assert!(matches!(result, Err(CarveError::TypeMismatch { .. })));
}
...
fn test_error_path_not_found() {
    ...
    let result = apply_patch(&doc, &patch);
    match result {
        Err(CarveError::PathNotFound { path }) => {
...
fn test_error_index_out_of_bounds_details() {
    ...
    let result = apply_patch(&doc, &patch);
    match result {
        Err(CarveError::IndexOutOfBounds { path, index, len }) => {
...
fn test_error_display_format() {
    let err = CarveError::path_not_found(path!("foo", "bar"));
    let msg = format!("{}", err);
