...
///
/// This keeps current behavior unchanged:
/// - execution patches are preserved as independent tracked patches
/// - plugin pending patches are flattened into one tracked patch
/// - optional cross-call conflict detection runs before merge
///
/// The helper centralizes parallel-merge behavior so the runtime can evolve
/// toward action-first conflict resolution in follow-up changes.
...
mod tests {
    ...
    fn merge_parallel_state_patches_rejects_conflicts_when_enabled() {
        ...
        let err = merge_parallel_state_patches(&[left, right], true).expect_err("must conflict");
        match err {
    ...
    fn merge_parallel_state_patches_skips_conflict_check_when_disabled() {
        ...
        let patches =
            merge_parallel_state_patches(&[left, right], false).expect("merge should succeed");
