                ...
                .await
            {
                match existing.status {
                    ...
                    | SubAgentStatus::Failed => {
                        ...
                        {
                            Ok(handle) => handle,
                            Err(err) => return Ok(tool_error(tool_name, "unknown_run", err)),
                        };
                        return Ok(self
                            .persist_summary(
                                ctx,
                    ...
                    SubAgentStatus::Stopped => {
                        ...
                        {
                            return Ok(err.into_tool_result(tool_name));
                        }
    ...
    async fn execute(
        ...
        for id in work_ids {
            ...
            if let Some(run) = persisted {
                ...
                if let Err(err) = ctx
                    .state_of::<SubAgentState>()
                    .runs_insert(id.clone(), updated.clone())
                {
                    return Ok(state_write_failed(tool_name, err));
                }
            }
...
impl Tool for AgentOutputTool {
    ...
    async fn execute(
            ...
            && sub.execution.output_thread_id().is_none()
        {
            ...
            if let Err(err) = ctx
                .state_of::<SubAgentState>()
                .runs_insert(run_id.clone(), sub.clone())
            {
                return Ok(state_write_failed(tool_name, err));
            }
        }
        ...
        let output = thread.and_then(|head| {
            head.thread
                .messages
                .iter()
                .rev()
