...
async fn test_concurrent_errors_dont_corrupt_storage() {
    ...
    for i in 0..50 {
        let storage = Arc::clone(&storage);
        let handle: tokio::task::JoinHandle<
            Result<Option<Thread>, tirea_agentos::contracts::storage::AgentStateStoreError>,
        > = tokio::spawn(async move {
            if i % 3 == 0 {
                // Load
...
async fn test_file_storage_corrupted_json() {
    ...
    match result {
        Err(tirea_agentos::contracts::storage::AgentStateStoreError::Serialization(msg)) => {
...
use tirea_protocol_ag_ui::AGUIMessage;
...
fn test_base_event_fields_with_raw_event() {
    use tirea_protocol_ag_ui::BaseEventFields;
...
fn test_event_with_raw_event_passthrough() {
    ...
    let event = AGUIEvent::TextMessageContent {
        ...
        delta: "Hello".into(),
        base: tirea_protocol_ag_ui::BaseEventFields {
            timestamp: Some(1234567890),
