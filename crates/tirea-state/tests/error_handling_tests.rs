...
fn test_error_increment_non_number() {
    ...
    let patch = Patch::new().with_op(Op::Increment {
        path: path!("value"),
        amount: tirea_state::Number::Int(1),
    });
...
#[test]
fn test_tirea_error_path_not_found() {
    let err = TireaError::path_not_found(path!("some", "path"));
    let err_str = err.to_string();
    assert!(err_str.contains("some") || err_str.contains("path"));
}

#[test]
fn test_tirea_error_type_mismatch() {
    let err = TireaError::type_mismatch(path!("test"), "string", "integer");
    let err_str = err.to_string();
    assert!(err_str.contains("string") || err_str.contains("integer"));
}

#[test]
fn test_tirea_error_index_out_of_bounds() {
    let err = TireaError::index_out_of_bounds(path!("arr"), 10, 5);
    let err_str = err.to_string();
