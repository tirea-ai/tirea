...
- tools: `agent_run`, `task_status`, `task_cancel`, `task_output`
- behaviors: `agent_tools`, `agent_recovery`, `background_tasks`
...
- inspect background run state: `task_status`
- cancel background run: `task_cancel` (descendants are cancelled automatically)
- fetch output snapshot: `task_output`

3. Choose foreground/background execution per `agent_run` call.

- `background=false`: parent waits and receives child progress
- `background=true`: child runs asynchronously and can be resumed/cancelled later

## Verify

- Orchestrator can call `agent_run` for allowed child agents.
- Child run status transitions are visible (`running`, `completed`, `failed`, `cancelled`, `stopped`).
- `task_output` returns child-thread outputs for the requested `task_id` (`run_id` from `agent_run`).
...
- Background runs left running without `task_cancel`/resume policy.
