...
import { PlaceCard } from "@/features/travel/components/PlaceCard";
...
function highlightPlace(lat: number, lng: number) {
  window.dispatchEvent(new CustomEvent("highlight-place", { detail: { lat, lng } }));
}
...
export function TripSidebar({ trips, selectedTripId, onSelectTrip }: TripSidebarProps) {
  ...
  return (
    <div style={{ padding: 16, overflowY: "auto", flex: "0 0 auto", maxHeight: "50%" }}>
      ...
      {trips.length === 0 && (
        <p style={{ color: "#888" }}>No trips yet. Ask the assistant to plan a trip!</p>
      )}
      {selectedTrip ? (
        <div>
