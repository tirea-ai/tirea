...
pub enum StateError {
    ...
    #[error("slot already registered: {key}")]
    SlotAlreadyRegistered { key: String },
    #[error("unknown slot: {key}")]
    UnknownSlot { key: String },
    #[error("failed to decode slot {key}: {message}")]
    SlotDecode { key: String, message: String },
